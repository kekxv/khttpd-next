...
TEST(HttpRouterTest, HeadFallsBackToGetHandler)
{
  khttpd_fw::HttpRouter router;
  bool called = false;

  router.get("/resource", [&](khttpd_fw::HttpContext& ctx)
  {
    called = true;
    ctx.set_status(http::status::ok);
    ctx.set_body("body generated by get");
  });

  http::request<http::string_body> req = make_request(http::verb::head, "/resource");
  http::response<http::string_body> res;
  khttpd_fw::HttpContext ctx = create_http_context(req, res);
